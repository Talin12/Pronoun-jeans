...
const EmptyCart = ({ navigate }) => (
  <div className="flex flex-col items-center justify-center py-28 gap-4">
    <div className="w-20 h-20 rounded-full bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-white/5 flex items-center justify-center">
    ...
    <tr className={`border-b border-gray-100 dark:border-white/5 ${index % 2 === 0 ? 'bg-gray-50/50 dark:bg-white/[0.015]' : 'bg-white dark:bg-transparent'}`}>
      <td className="px-6 py-4">
        <div className="flex items-start gap-3">
          <div className="w-9 h-9 rounded-lg bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 flex items-center justify-center shrink-0">
            <Tag className="w-4 h-4 text-gray-400 dark:text-zinc-600" />
          </div>
          <div>
      ...
      <td className="px-6 py-4 text-gray-900 dark:text-zinc-100 font-bold text-sm whitespace-nowrap">₹{subtotal}</td>
      <td className="px-6 py-4">
        <button onClick={() => onQtyChange(id, 0)} className="text-gray-400 dark:text-zinc-600 hover:text-red-500 transition-colors">
    ...
    <div onClick={() => onSelect(addr.id)}
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          <Icon className={`w-3.5 h-3.5 ${selected ? 'text-accent' : 'text-gray-400 dark:text-zinc-500'}`} />
          <span className={`text-xs font-bold uppercase tracking-widest ${selected ? 'text-accent' : 'text-gray-500 dark:text-zinc-500'}`}>
      ...
      {addr.address_line_2 && <p className="text-gray-500 dark:text-zinc-400 text-xs">{addr.address_line_2}</p>}
      <p className="text-gray-500 dark:text-zinc-400 text-xs">{addr.city}, {addr.state} — {addr.pincode}</p>
    </div>
  );
};
            ...
            {PAYMENT_OPTIONS.map(opt => {
              ...
              return (
                <label key={opt.value} onClick={() => onPaymentSelect(opt.value)}
                  ...
                  <div className={`w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center ${active ? 'border-accent' : 'border-gray-300 dark:border-zinc-600'}`}>
                    {active && <div className="w-2 h-2 rounded-full bg-accent" />}
                  </div>
                  <Icon className={`w-4 h-4 shrink-0 ${active ? 'text-accent' : 'text-gray-400 dark:text-zinc-500'}`} />
                  <div>
                    <p className={`text-sm font-bold ${active ? 'text-accent' : 'text-gray-900 dark:text-zinc-100'}`}>{opt.label}</p>
                    <p className="text-gray-500 dark:text-zinc-500 text-xs">{opt.subtitle}</p>
                  </div>
                </label>
              );
            })}
