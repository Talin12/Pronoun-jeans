...
// Read once, as the module loads — before React mounts and empties #root.
const BOOTSTRAP = readBootstrap('home');

// Always-present LCP candidate. The hero URL normally arrives from the API, so
// without this the first paint of a cold client-side visit is a grey pulse:
// nothing for the browser to measure, and LCP waits on the network.
// Generated by scripts/generate-hero-fallback.py from the first hero slide.
const FALLBACK_HERO = '/hero-fallback.jpg';
const FALLBACK_HERO_ALT = "Pronoun Jeans wholesale men's bottomwear, manufactured in Ahmedabad";

// `sizes="100vw"` is wrong for this image, and measurably so.
//
// The hero is object-cover in a box 92vh tall. On a phone that box is portrait
// while the photograph is about 2.6:1 landscape, so covering it renders the
// image roughly 1980 CSS px wide and crops the sides — the browser paints far
// more image than the viewport is wide. Asked for 100vw it fetched the 800w
// file, upscaled it, and Chrome then discounted the LCP candidate for being
// lower-resolution than the area it filled: 239k against the 306k the box is
// worth. The *second* slide happened to be higher-resolution, scored the full
// 306k, and superseded it — which is why LCP was reported at 11.9s, the moment
// the carousel advanced, rather than at 3.0s when the hero actually appeared.
//
// 686 CSS px lands on the 1200w variant at a 1.75 DPR, which Cloudinary serves
// as WebP at 60KB — a kilobyte *less* than the 800w JPEG it replaces. Sharper
// hero, same bytes, and no later slide can outscore the first.
const HERO_RESOLUTION = {
  sizes: '(max-width: 640px) 686px, 100vw',
...
const HeroSlideshow = ({ slides }) => {
  ...
  // A real <img>, not the old gradient: this is the LCP element whenever the
  // slides have not arrived, and a gradient is not a contentful paint.
  if (slides.length === 0) {
    return (
    ...
    <>
      ...
      <div className={`absolute inset-0 transition-opacity duration-500 ease-in-out ${fading ? 'opacity-0' : 'opacity-100'}`}>
        <ResponsiveImage
          key={slide.id}
          src={slide.image}
          // "Slide 2" describes nothing. Captions are optional in the admin,
          // so the fallback says what the image actually shows.
          alt={slide.caption || FALLBACK_HERO_ALT}
          priority
          {...HERO_RESOLUTION}
          className="w-full h-full object-cover object-center"
        />
...
const Home = () => {
  ...
  // Seeded from what the prerenderer captured, so the hero is on screen in the
  // first paint instead of after a round trip to a cold Render instance. The
  // fetches below still run and overwrite this with whatever is current.
  const [categories, setCategories]     = useState(BOOTSTRAP?.categories ?? []);
  const [slides, setSlides]             = useState(BOOTSTRAP?.slides ?? []);
  const [slidesLoaded, setSlidesLoaded] = useState(Boolean(BOOTSTRAP));
  const [catsLoaded, setCatsLoaded]     = useState(Boolean(BOOTSTRAP));
  ...
  return (
    <div className="bg-white dark:bg-zinc-950 min-h-screen">
      ...
      <BootstrapData id="home" data={{ slides, categories }} />
